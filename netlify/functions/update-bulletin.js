...
exports.handler = async function () {
  ...
  try {
    ...
    console.log('[update-bulletin] Recherche du dernier bulletin SPF…');
    const pdfUrl = await findLatestBulletinUrl();
    console.log('[update-bulletin] PDF trouvé :', pdfUrl);

    /* 2. Télécharger le PDF */
    console.log('[update-bulletin] Téléchargement…');
    ...
    console.log('[update-bulletin] Extraction via Gemini…');
    const extracted = await extractWithGemini(pdfBase64);
    console.log('[update-bulletin] Données :', JSON.stringify({ semaine: extracted.semaine, passages_total: extracted.passages_total }));
    ...
    console.log('[update-bulletin] ✅ Stocké dans Netlify Blobs');
