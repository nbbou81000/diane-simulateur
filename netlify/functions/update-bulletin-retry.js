...
async function findPdfUrl() {
  ...
  console.log('[update-bulletin-retry] Page SPF status:', mainPage.status, '— taille:', mainPage.body.length, 'chars');
  ...
  if (directPdf.length) {
    ...
    console.log('[update-bulletin-retry] PDF direct trouvé:', url);
    return url;
  }

  /* Chercher lien vers page bulletin */
  const bulletinLinks = [...mainPage.body.matchAll(/href="([^"]*bulletin[^"]*oscour[^"]*)"/gi)];
  console.log('[update-bulletin-retry] Liens bulletin trouvés:', bulletinLinks.length);
  ...
  console.log('[update-bulletin-retry] Page bulletin intermédiaire:', bulletinPageUrl);

  const bulletinPage = await httpsGet(bulletinPageUrl);
  const pdfs = [...bulletinPage.body.matchAll(/href="([^"]*\.pdf[^"]*)"/gi)];
  console.log('[update-bulletin-retry] PDFs trouvés sur page bulletin:', pdfs.length);
...
exports.handler = async function () {
  ...
  try {
    ...
    if (existing) {
      ...
      if (ageH < 120) {
        console.log('[update-bulletin-retry] Frais (' + Math.round(ageH) + 'h) — skip');
  ...
  try {
    ...
    console.log('[update-bulletin-retry] PDF URL finale:', pdfUrl);
    ...
    console.log('[update-bulletin-retry] Extrait:', data.semaine, '— passages:', data.passages_total);

    /* 4. Stocker */
    const store = getStore('oscour-bulletin');
    await store.set('latest', JSON.stringify({ ...data, pdf_url: pdfUrl, updated_at: new Date().toISOString() }));
    console.log('[update-bulletin-retry] ✅ Stocké');
