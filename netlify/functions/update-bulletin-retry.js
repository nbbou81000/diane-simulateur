...
async function findLatestPdf() {
  ...
  console.log('[update-bulletin-retry] Dates candidates:', dates.join(', '));
  ...
  for (const date of dates) {
    for (const makeUrl of patterns) {
      ...
      try {
        console.log('[update-bulletin-retry] Test:', url);
        ...
        if (check.status === 200) {
          console.log('[update-bulletin-retry] ✅ PDF trouvé:', url);
          return url;
        }
        if ([301,302].includes(check.status) && check.location) {
          ...
          if (finalUrl.endsWith('.pdf')) {
            console.log('[update-bulletin-retry] ✅ PDF via redirect:', finalUrl);
            return finalUrl;
          }
        }
      } catch(_) { /* continuer */ }
...
async function scrapeBulletinPage() {
  ...
  console.log('[update-bulletin-retry] Scraping page liste:', listUrl);
  const page = await httpsGet(listUrl);
  console.log('[update-bulletin-retry] Status:', page.status, '— taille:', page.body.length);
  ...
  if (allPdfs.length) {
    ...
    console.log('[update-bulletin-retry] PDF via scraping:', url);
    return url;
  }
